...
//
// 使い方:
//   node judge/j062_check.js --answer <答案.md> --transcript <transcript.jsonl> [--arm A2|A2b] [--run <走ID>] [--json]
...
function main() {
  ...
  if (!a.answer || !a.transcript) {
    console.error('usage: node judge/j062_check.js --answer <答案.md> --transcript <transcript.jsonl> [--arm A2|A2b] [--run <走ID>] [--json]');
    process.exit(2);
  }
  ...
  L(`# j062 突合 run=${result.run || '-'} card=G arm=${result.arm || '-'}`);
