#!/usr/bin/env node
// j065_check.js — job_065（箱 2026-09-25 起票）。土台＝j064_check.js（md5 b5d0bdc85d0771317e264756113beb98）の複写。
// 仕様の正は ORDER_20260925d_itte_doushi.md §3。事前ロック＝jobs/job_065_prelock.md §3。j064 は触らん。
// 足し分は末尾「065 の足し分」節（母艦 9/25）:
//   ⑭副6'＝md の語が見出し・定型ラベルとして写ったかの候補出し（行頭・太字・見出し行）。
//     語＝「変えること」「1つ変える」「1個」＋変形（変える…1点/1つ/1個 等）。A2f 走の名詞化「変えること」見出しも同じ網で拾う。
//     本文中の普通の文は 'body' として別記（数えるだけ・候補には入れん）。二値は人。
// 以下は j064 までの見出しコメント（記録として残す）:
...
//
// 使い方:
//   node judge/j065_check.js --answer <答案.md> --transcript <transcript.jsonl> [--arm A2d|A2e] [--run <走ID>] [--json]
...
// ---------- 副6': md の語が見出し・定型ラベルとして写ったか（ORDER 065 §3） ----------
// 逐語の語＋変形（「今回変える1点」「1つだけ変える」「変更は1個」等）。normalize 後（全角数字→半角）に当てる。
const SUB6P_RE = /変えること|何を1つ変えるか|1つ変える|一つ変える|1個|変え[るた][^\n]{0,6}(?:1|一)[つ個点]|(?:1|一)[つ個点][^\n]{0,4}変え|変更[^\n]{0,3}(?:1|一)[つ個点]/g;
// 行の種類: heading（# 見出し）／bold（**…** の中）／label（行頭ラベル＝箇条記号の後の短い語＋「：/:」、または 20 字以下の単独行）／body
function sub6pLineKind(line, m, idx) {
  if (/^\s*#{1,6}\s/.test(line)) return 'heading';
  const boldRe = /\*\*([^*]+)\*\*|__([^_]+)__/g;
  let b;
  while ((b = boldRe.exec(line))) { if (idx >= b.index && idx < b.index + b[0].length) return 'bold'; }
  const body = line.replace(/^\s*(?:[-*・]|\d+[.)、]|[①-⑳])\s*/, '');
  const colon = body.search(/[:：]/);
  const off = line.length - body.length;
  if (colon >= 0 && colon <= 20 && idx - off < colon) return 'label';
  if (body.trim().length <= 20) return 'label';
  return 'body';
}
function sub6pBlock(raw) {
  const lines = normalize(raw).split('\n');
  const hitsOut = [];
  for (const line of lines) {
    SUB6P_RE.lastIndex = 0;
    let m;
    while ((m = SUB6P_RE.exec(line))) hitsOut.push({ kind: sub6pLineKind(line, m[0], m.index), word: m[0], line: line.trim().slice(0, 80) });
  }
  const cand = hitsOut.filter((h) => h.kind !== 'body');
  return { candidate: cand.length > 0, hits: hitsOut, label_count: cand.length, body_count: hitsOut.length - cand.length };
}
...
function main() {
  ...
  if (!a.answer || !a.transcript) {
    console.error('usage: node judge/j065_check.js --answer <答案.md> --transcript <transcript.jsonl> [--arm A2e|A2f] [--run <走ID>] [--json]');
    process.exit(2);
  }
  ...
  // 065 走単位の集計（T1＋T2・材料）
  const s6p = bs.flatMap((b) => b.j065_sub6p.hits);
  result.j065_run = {
    sub6p_candidate: s6p.some((h) => h.kind !== 'body'),
    sub6p_kind_counts: s6p.reduce((m, h) => ((m[h.kind] = (m[h.kind] || 0) + 1), m), {}),
    sub6p_label_words: [...new Set(s6p.filter((h) => h.kind !== 'body').map((h) => h.word))],
    note: "⚠️全部材料。副6' の二値は人（F2/F4/F6 を先に付けてから）。body は候補に入れん。",
  };
  ...
  L(`# j065 突合 run=${result.run || '-'} card=G arm=${result.arm || '-'}`);
  ...
  for (const [nm, b] of [['T1', result.T1], ['T2', result.T2]]) {
    ...
    L(`   [065] 副6' 候補=${b.j065_sub6p.candidate ? 'yes' : 'no'}（ラベル ${b.j065_sub6p.label_count}／本文 ${b.j065_sub6p.body_count}）`);
    for (const h of b.j065_sub6p.hits) L(`      ${h.kind} [${h.word}] ${JSON.stringify(h.line)}`);
  ...
  const r9 = result.j065_run;
  L(`[065 走集計・材料] 副6' 候補=${r9.sub6p_candidate ? 'yes' : 'no'}（${JSON.stringify(r9.sub6p_kind_counts)}・ラベル語[${j(r9.sub6p_label_words)}]）`);
